...
- **IQ-02** [DONE] (P1·L) Embedding decision. — Evidence: ADR `docs/adr/0004-embedding-model.md`. 200 fixture pairs: full-text MRR 1.00, 8-token prefix MRR 0.50 (`scripts/retrieval_eval.py`). Default stays `all-MiniLM-L6-v2` because Nomic weights were not loaded; `EMBEDDING_CUTOVER_ENABLED` defaults false and the lite profile stays on MiniLM. HNSW on the 384-d column already exists from migration 0001. Resumable metadata backfill tested on 1,000 rows (`backfill_batch`); a 50k-row stamp was not executed. Rollback drops 0013's columns and tables and leaves vectors in place.
...
- **IQ-16** [DONE] (P0·L) Evaluation harness. — Evidence: `synthetic_eval` and `make eval` (`backend/scripts/rank_eval.py`). Report `docs/eval/ranking.md`: PRS NDCG@10 1.00, chronological 0.5459, semantic-only 0.5924, random 0.5160. Margin over chronological 0.45, above the 0.05 gate. `test_exploration_ipw_and_eval_gate` runs in CI.
...
**Gate 2 — Intelligence**
- [x] Eval report committed: `docs/eval/ranking.md`. PRS NDCG@10 1.00 vs chronological 0.5459, semantic-only 0.5924, random 0.5160.
- [x] ADR 0004. Fixture retrieval: full-text MRR 1.00, prefix MRR 0.50. Default stays MiniLM. Cutover flag defaults off. Migration 0013 downgrade drops the new columns and tables.
