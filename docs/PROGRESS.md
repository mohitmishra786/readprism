...
- **IN-07** [TODO] (P1·M) Canonicalization + dedupe: strip tracking params (`utm_*`, `fbclid`, `gclid`, …), lowercase host, honor `<link rel=canonical>`/`og:url`; identity order: (source,guid) → canonical URL → content simhash; cross-source near-duplicate via embedding sim + title sim (thresholds configurable). — Accept: fixture set of duplicates/syndications collapses correctly; no false merges on the negative set. — Deps: IQ-01 for embedding part
