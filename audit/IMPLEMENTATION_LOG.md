...
- [x] 09-4 | P2 | Code | Dark-mode coverage on the inline-styled pages (owner asked to do deferrals): converted the neutral text/border/background hexes in Preferences + Onboarding (Wizard/SampleArticles/InterestInput) to the semantic CSS-variable tokens that flip in dark mode, keeping brand/viz colors. tsc/build clean. (A full Tailwind-class migration is still optional polish.)
